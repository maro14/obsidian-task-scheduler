...
export class SchedulerView extends ItemView {
    ...
    private renderView(): void {
        ...
        // Create header
        const header = container.createEl('div', { cls: 'task-scheduler-header' });
        header.createEl('h2', { text: 'Task Schedule' });

        // Create refresh button
        const refreshButton = header.createEl('button', { cls: 'task-scheduler-refresh-button', text: 'Refresh' });
    ...
    private renderTaskItem(taskList: HTMLElement, task: Task): void {
        const taskItem = taskList.createEl('li', { cls: 'task-scheduler-task-item' });
        ...
        const metadata = taskItem.createEl('div', { cls: 'task-scheduler-task-metadata' });
        ...
        if (task.metadata.deadline) {
            ...
            if (task.isOverdue()) {
                ...
                deadlineEl.setText(`Overdue: ${task.metadata.deadline.toLocaleDateString()}`);
            } else if (task.isDueSoon()) {
        ...
        // Action buttons
        const actionsEl = taskItem.createEl('div', { cls: 'task-scheduler-task-actions' });
        ...
        openSourceBtn.addEventListener('click', () => {
            ...
            this.app.workspace.openLinkText(
                ...
                { line: task.metadata.lineNumber } // Open at specific line
            );
        });
    }
    ...
    private async renderStatistics(container: HTMLElement): Promise<void> {
        ...
        const statsContainer = container.createEl('div', { cls: 'task-scheduler-statistics' });
        
        const statsHeader = statsContainer.createEl('h3', { text: 'Schedule Statistics' });
        ...
        totalTasksEl.createEl('span', { cls: 'stat-value', text: `${stats.totalTasks}` });
        
        // Scheduled tasks
        const scheduledTasksEl = statsList.createEl('div', { cls: 'stat-item' });
        scheduledTasksEl.createEl('span', { cls: 'stat-label', text: 'Scheduled:' });
        scheduledTasksEl.createEl('span', { cls: 'stat-value', text: `${stats.scheduledTasks}` });
        
        // Completed tasks
        const completedTasksEl = statsList.createEl('div', { cls: 'stat-item' });
        completedTasksEl.createEl('span', { cls: 'stat-label', text: 'Completed:' });
        completedTasksEl.createEl('span', { cls: 'stat-value', text: `${stats.completedTasks}` });
        
        // Upcoming deadlines
        const upcomingDeadlinesEl = statsList.createEl('div', { cls: 'stat-item' });
        upcomingDeadlinesEl.createEl('span', { cls: 'stat-label', text: 'Upcoming Deadlines:' });
    ...
    private renderCalendarView(container: HTMLElement): void {
        const calendarContainer = container.createEl('div', { cls: 'task-scheduler-calendar' });
        ...
        for (const [dateStr, slots] of slotsByDay.entries()) {
            ...
            const dayEl = calendarContainer.createEl('div', { cls: 'task-scheduler-day' });
    ...
    private renderTaskList(container: HTMLElement): void {
        const taskListContainer = container.createEl('div', { cls: 'task-scheduler-task-list' });
        taskListContainer.createEl('h3', { text: 'Scheduled Tasks' });

        // Create filter controls
        const filterContainer = taskListContainer.createEl('div', { cls: 'task-scheduler-filters' });
        
        // Category filter
        const categoryFilter = filterContainer.createEl('select', { cls: 'task-scheduler-category-filter' });
        ...
        // Tag filter
        const tagFilter = filterContainer.createEl('select', { cls: 'task-scheduler-tag-filter' });
        ...
        const sortedTasks = [...this.scheduledTasks].sort((a, b) => {
            ...
            return a.metadata.scheduledTime.getTime() - b.metadata.scheduledTime.getTime();
        });
        ...
        const renderFilteredTasks = () => {
            ...
            const filteredTasks = sortedTasks.filter(task => {
                // Category filter
                if (selectedCategory !== 'all' && task.metadata.category !== selectedCategory) {
                    return false;
                }
                
                // Tag filter
                if (selectedTag !== 'all' && !task.metadata.tags.includes(selectedTag)) {
                    return false;
                }
            ...
            if (filteredTasks.length === 0) {
                taskList.createEl('li', { text: 'No tasks match the selected filters' });
                return;
            }
