...
export class TasksPluginIntegration {
     ...
     */
    async getTasks(defaultPriority: number, defaultTimeEstimate: number): Promise<Task[]> {
        ...
        try {
            ...
            return tasksCacheApi
                ...
                .map((taskItem: any) => this.convertTasksPluginTask(taskItem, defaultPriority, defaultTimeEstimate));
        } catch (error) {
     ...
     */
    private convertTasksPluginTask(taskItem: any, defaultPriority: number, defaultTimeEstimate: number): Task {
        try {
    ...
    async updateTask(task: Task): Promise<boolean> {
        ...
        try {
            ...
            const taskItem = tasksCacheApi.find((t: any) => 
    ...
    async createTask(task: Task, targetFile?: TFile): Promise<boolean> {
        ...
        try {
            ...
            if (!targetFile) {
                ...
                if (tasksSettings && tasksSettings.globalFilter && tasksSettings.globalFilter.sourcePath) {
                    const defaultTasksPath = tasksSettings.globalFilter.sourcePath;
                    targetFile = this.app.vault.getAbstractFileByPath(defaultTasksPath) as TFile;
                }
